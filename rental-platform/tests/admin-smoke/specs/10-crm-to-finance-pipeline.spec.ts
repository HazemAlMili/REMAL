...
test.describe("Cross-Module CRM-to-Finance Pipeline Loop", () => {
  test("E2E conversion from Lead to Booking to Invoice and Payout Ledger calculations", async ({
    superAdminPage,
    request,
  }) => {
    ...
    const contactedBtn = superAdminPage
      .locator('button:has-text("Contacted")')
      .first();
    await contactedBtn.click();
    await superAdminPage.click('button:has-text("Confirm")');
    await expect(
      superAdminPage.locator('span:has-text("Contacted")').first()
    ).toBeVisible({ timeout: 10000 });

    const qualifiedBtn = superAdminPage
      .locator('button:has-text("Qualified")')
      .first();
    await qualifiedBtn.click();
    await superAdminPage.click('button:has-text("Confirm")');
    await expect(
      superAdminPage.locator('span:has-text("Qualified")').first()
    ).toBeVisible({ timeout: 10000 });

    // 5. Convert lead to Booking using the ConvertToBookingPanel
    await superAdminPage.fill(
      'input[label="Client ID"], input[placeholder*="client ID"]',
      "aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa"
    );
    await superAdminPage.click('button:has-text("Convert to Booking")');

    // Verification: Should redirect to Booking Detail Page
    await superAdminPage.waitForURL(/\/admin\/bookings\/[a-f0-9-]/, {
      timeout: 15000,
    });
    await expect(superAdminPage.locator("h1")).toHaveText(/Booking/i);

    // 6. Verify invoice auto-generation pattern: INV-{YEAR}{MONTH}-
    const invoiceTab = superAdminPage.locator(
      'button:has-text("Invoice"), a:has-text("Invoice")'
    );
    ...
    await expect(superAdminPage.locator("body")).toContainText(invoicePattern, {
      timeout: 15000,
    });
  });
});
