...
test.describe("Role-Based Access Control (RBAC) Verification", () => {
  // --- SALES ROLE ---
  test("Sales role has access to CRM but not Finance", async ({
    salesPage,
  }) => {
    await salesPage.goto("/admin/dashboard");

    const crmLink = salesPage
      .getByRole("navigation")
      .getByRole("link", { name: "Leads pipeline" });
    const financeLink = salesPage
      .getByRole("navigation")
      .getByRole("link", { name: "Finance hub" });
  ...
  // --- FINANCE ROLE ---
  test("Finance role has access to Finance but not CRM", async ({
    financePage,
  }) => {
    await financePage.goto("/admin/dashboard");

    const financeLink = financePage
      .getByRole("navigation")
      .getByRole("link", { name: "Finance hub" });
    const crmLink = financePage
      .getByRole("navigation")
      .getByRole("link", { name: "Leads pipeline" });
  ...
  // --- TECH ROLE ---
  test("Tech role has access to Units setup but not Finance", async ({
    techPage,
  }) => {
    await techPage.goto("/admin/dashboard");

    const unitsLink = techPage
      .getByRole("navigation")
      .getByRole("link", { name: "Units", exact: true });
    const financeLink = techPage
      .getByRole("navigation")
      .getByRole("link", { name: "Finance hub" });
  ...
  // --- SUPERADMIN ROLE ---
  test("SuperAdmin role has unfettered access across all modules", async ({
    superAdminPage,
  }) => {
    ...
    await expect(
      nav.getByRole("link", { name: "Leads pipeline" })
    ).toBeVisible();
    ...
    await expect(
      nav.getByRole("link", { name: "Resort projects" })
    ).toBeVisible();
