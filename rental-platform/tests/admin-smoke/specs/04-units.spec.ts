...
test.describe("Units Inventory & Calendar Management", () => {
  test("Can create a unit, toggle status, and create date blocks that show up in availability calendar", async ({
    superAdminPage,
    request,
  }) => {
    ...
    await superAdminPage.click(
      'div[class*="cursor-pointer"]:has-text("Ahmed Hassan")'
    );

    // Select Project (the first select on page)
    ...
    await superAdminPage.waitForURL(/\/admin\/units\/[a-f0-9-]/, {
      timeout: 15000,
    });
    ...
    await expect(dateBlockRow).toContainText(
      "E2E Scheduled Maintenance Closure"
    );
    ...
    await expect(
      superAdminPage.locator('h2:has-text("Unit Availability")')
    ).toBeVisible();

    // Verify cell 15 is marked as unavailable
    const cell15 = superAdminPage
      .locator('div[class*="bg-red-50"]:has-text("15")')
      .first();
