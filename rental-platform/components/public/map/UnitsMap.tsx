...
import {
  ...
  MAP_CENTER,
  MAP_ZOOM,
} from "@/lib/constants/project-coordinates";
...
export function UnitsMap({ projects }: UnitsMapProps) {
  ...
  // Add markers when map is loaded and projects are available
  useEffect(() => {
    ...
    projects.forEach((project) => {
      const coords = getProjectCoordinates(project.id);
      if (!coords) return; // skip projects without coordinates

      // Custom marker element — terracotta circle with pulsing ring
      const el = document.createElement("div");
      el.className = "mapbox-project-marker";
      el.innerHTML = `
        <div class="relative">
      ...
      const popup = new mapboxgl.Popup({
        ...
        <div style="font-family: Inter, sans-serif; padding: 4px 0;">
          ...
          ${
            project.description
              ? `<p style="font-size: 12px; color: #666; margin: 0 0 8px 0; line-height: 1.4;">${project.description.slice(0, 80)}${project.description.length > 80 ? "..." : ""}</p>`
              : ""
          }
          <a
  ...
  if (!MAPBOX_TOKEN) {
    return (
      <div className="flex h-[400px] w-full items-center justify-center rounded-lg border border-stone-200 bg-stone-100 lg:h-[500px]">
        <div className="space-y-1 text-center text-stone-400">
