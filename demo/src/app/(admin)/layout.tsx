    ...
    <div className="flex flex-col md:flex-row h-screen bg-gray-50 text-gray-900 overflow-hidden font-sans">
      ...
      <header className="md:hidden flex items-center justify-between px-4 h-16 bg-white border-b border-gray-200 sticky top-0 z-40 shrink-0 shadow-sm">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded-lg bg-brand-900 flex items-center justify-center text-white font-bold text-sm">
            KB
          </div>
          <span className="font-black text-brand-950 tracking-tight">
            Admin
          </span>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="w-10 h-10 flex items-center justify-center bg-gray-100 rounded-full text-brand-900 border border-gray-200"
          >
      ...
      <aside className="hidden md:flex w-[80px] lg:w-72 bg-white border-l border-gray-200 flex-col h-full shadow-soft z-20 relative shrink-0 transition-all duration-300">
        <div className="h-20 flex items-center justify-center lg:justify-start lg:px-8 border-b border-gray-100 shrink-0">
          <Link
            ...
            className="font-bold text-xl lg:text-2xl tracking-tight text-brand-900 flex items-center gap-3"
          >
            ...
            return (
              <Link
                ...
                className={cn(
                  ...
                  isActive
                    ? "bg-brand-50 text-brand-700 shadow-[inset_0_-2px_4px_rgba(0,0,0,0.02)]"
                    : "text-gray-600 hover:bg-gray-50 hover:text-gray-900",
                )}
              >
                {isActive && (
                  <div className="absolute right-1 lg:right-0 top-1/2 -translate-y-1/2 w-1 lg:w-1 h-4 lg:h-full bg-brand-600 rounded-full" />
                )}
                <item.icon
                  className={cn(
                    "w-6 h-6 lg:w-5 lg:h-5 shrink-0 transition-transform",
                    isActive
                      ? "text-brand-600"
        ...
        <div className="p-4 border-t border-gray-100 lg:block flex justify-center shrink-0">
          <button
            ...
            className="flex items-center justify-center lg:justify-start gap-3 h-12 lg:h-auto lg:px-4 lg:py-3 rounded-full lg:rounded-xl font-medium text-red-600 hover:bg-red-50 w-12 lg:w-full transition-all"
          >
          ...
          <div className="flex items-center gap-6 shrink-0">
            ...
            <div className="flex items-center gap-3 cursor-pointer">
              ...
              <div className="flex flex-col hidden lg:flex">
                <span className="font-bold text-sm leading-tight text-gray-900 whitespace-nowrap">
                  محمد علي
                </span>
          ...
          >
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-3 mb-6">
                {MENU_ITEMS.filter((item) => item.hideOnMobile).map(
                  (item, idx) => (
                    <Link
                      key={idx}
                      href={item.href}
                      onClick={() => setIsMobileMenuOpen(false)}
                      className="flex items-center gap-3 p-3 bg-gray-50 rounded-2xl hover:bg-gray-100 text-gray-900 transition-colors"
                    >
                      <div className="bg-brand-100/50 p-2 rounded-xl">
                        <item.icon
              ...
              <div className="mt-auto border-t border-gray-100 pt-4 flex flex-col gap-2">
                <Link
                  href={ROUTES.adminSettings}
                  onClick={() => setIsMobileMenuOpen(false)}
                  className="flex items-center justify-center gap-2 font-bold py-3 rounded-2xl bg-gray-50 text-gray-900"
                >
