...
export default function UnitsManagementPage() {
  ...
  const filtered = MOCK_UNITS.filter(
    (u) =>
      (u.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
        u.owner.name.includes(searchTerm)) &&
      (projectFilter === "" || u.projectId === projectFilter),
  );
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-500">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pt-2">
        <div>
          <h1 className="text-3xl font-black text-brand-950 mb-1">
            الوحدات العقارية
          </h1>
      ...
      <div className="bg-white rounded-[1.5rem] border border-gray-100 overflow-hidden shadow-[0_4px_24px_rgba(0,0,0,0.02)]">
        <table className="w-full text-right">
          <thead>
            <tr className="border-b border-gray-50">
              ...
              <th className="px-6 py-4 text-xs font-bold text-gray-400 uppercase tracking-wider hidden lg:table-cell">
                المالك
              </th>
              <th className="px-6 py-4 text-xs font-bold text-gray-400 uppercase tracking-wider">
                السعر
              ...
              >
                ...
                <td className="px-6 py-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-bold text-brand-950 text-sm leading-tight">
                        {unit.name}
                      </p>
                ...
                <td className="px-6 py-4">
                  <Badge
                    variant={unit.status === "active" ? "success" : "warning"}
        ...
        <div className="px-6 py-4 border-t border-gray-50 flex justify-between items-center">
          <p className="text-sm text-gray-400 font-medium">
            عرض {filtered.length} من {MOCK_UNITS.length} وحدة
          </p>
          <div className="flex gap-1">
            {[1, 2, 3].map((p) => (
              <button
                key={p}
                className={`w-8 h-8 rounded-lg text-sm font-bold transition-all ${p === 1 ? "bg-brand-950 text-white" : "text-gray-400 hover:bg-gray-100"}`}
              >
                {p}
              </button>
            ))}
