...
const MENU_ITEMS = [
  ...
  { icon: Wallet, label: "أرباحي", href: ROUTES.ownerEarnings },
  {
    icon: CalendarDays,
    label: "التقويم",
    href: ROUTES.ownerCalendar,
    hiddenOnMobile: true,
  },
  {
    icon: Bell,
    label: "الإشعارات",
    href: ROUTES.ownerNotifications,
    hiddenOnMobile: true,
  },
];
    ...
    <div className="flex flex-col md:flex-row h-screen bg-[#F7F7F9] text-brand-950 overflow-hidden font-sans dir-rtl">
      ...
      <header className="md:hidden flex items-center justify-between px-5 h-16 bg-white border-b border-gray-100 sticky top-0 z-40 shrink-0">
        <div className="flex items-center gap-2">
          ...
          <span className="text-accent-600 text-[9px] font-bold uppercase tracking-widest bg-accent-50 px-2 py-1 rounded-md">
            مُلاك
          </span>
        </div>
        <button
      ...
      <aside className="hidden md:flex flex-col w-[80px] lg:w-[260px] bg-white h-full z-20 relative border-l border-gray-100 shadow-[4px_0_24px_rgba(0,0,0,0.02)] transition-all duration-300 shrink-0">
        ...
        <div className="h-20 lg:h-24 flex flex-col justify-center items-center lg:items-start lg:px-8 relative z-10 pt-2 lg:pt-4">
          ...
          <span className="font-black text-2xl text-brand-950 lg:hidden">
            KB
          </span>
          <span className="text-accent-600 text-[10px] font-bold uppercase tracking-[0.2em] mt-1 lg:block hidden">
            بوابة المُلاك
          </span>
        </div>
          ...
          {MENU_ITEMS.map((item, idx) => {
            ...
            return (
              ...
              >
                ...
                <item.icon
                  className={cn(
                    "w-[22px] h-[22px] lg:w-5 lg:h-5 transition-transform duration-300 shrink-0",
                    isActive ? "text-brand-950" : "group-hover:scale-110",
                  )}
                />
                <span
                  className={cn(
                    "text-sm hidden lg:block truncate",
        ...
        <div className="p-4 lg:p-6 relative z-10 border-t border-gray-50 flex flex-col items-center lg:items-stretch">
          <div className="flex items-center lg:justify-between mb-4 lg:px-2 w-full justify-center">
            <div className="flex-col hidden lg:flex overflow-hidden">
              <span className="font-black text-sm text-brand-950 truncate">
                أحمد منصور
              </span>
              <span className="text-[11px] font-bold text-gray-400 mt-0.5 truncate">
                3 وحدات نشطة
              </span>
            </div>
            <div className="w-10 h-10 rounded-full bg-brand-50 border border-brand-100 flex items-center justify-center font-bold text-brand-700 shrink-0">
              أ.م
            </div>
          </div>
          <button
            ...
            className="flex items-center justify-center gap-2 text-sm font-bold text-gray-400 hover:text-red-500 lg:hover:bg-red-50 lg:w-full w-10 h-10 lg:h-auto lg:py-3 rounded-full lg:rounded-xl transition-all"
            title="تسجيل الخروج"
          >
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 flex items-center px-2 pt-2 pb-2 h-[76px] z-40 shadow-[0_-8px_30px_rgba(0,0,0,0.06)]">
        <div className="flex w-full items-center justify-between px-2">
          {MENU_ITEMS.filter((item) => !item.hiddenOnMobile).map(
            (item, idx) => {
              const isActive =
                pathname === item.href || pathname?.startsWith(item.href + "/");
              return (
                ...
                >
                  <div
                    className={cn(
                      "flex flex-col items-center justify-center p-1.5 px-4 rounded-full transition-all duration-300",
                      isActive ? "bg-brand-50 text-brand-950" : "text-gray-400",
                    )}
                  >
                    <item.icon
                      className={cn(
          ...
          >
            ...
            >
              ...
              <div className="flex items-center gap-4 mb-8 p-4 bg-brand-50/50 rounded-2xl border border-brand-50">
                <div className="w-14 h-14 rounded-full bg-white border border-brand-100 flex items-center justify-center font-bold text-xl text-brand-900 shadow-sm">
                  أ.م
                </div>
                <div>
                  <h3 className="font-black text-lg text-brand-950">
                    أحمد منصور
                  </h3>
                ...
                {MENU_ITEMS.filter((item) => item.hiddenOnMobile).map(
                  (item, idx) => (
                    <Link
                      ...
                      href={item.href}
                      onClick={() => setIsMobileMenuOpen(false)}
                      className="flex items-center gap-4 p-4 rounded-xl hover:bg-gray-50 text-brand-900 transition-colors"
                    >
